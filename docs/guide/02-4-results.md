...
在建立完整且无错误的输入控制文件后，即可运行模型并检查结果。

SCRAM 网站提供的个人计算机可执行程序会打开输入控制文件和打印输出文件。可以通过命令提示符使用以下三种方式执行模型：
...
后两种方式适用于 18081 及之后的 AERMOD 版本。用户可以指定输入控制文件名，也可以进一步指定输出文件名。
...
- 当命令中未指定输入控制文件名时，`AERMOD.INP` 必须位于模型当前执行目录中。
